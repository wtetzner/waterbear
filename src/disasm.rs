use crate::ast::{ByteValue, Directive, Statement};
...
pub fn disassemble(
    ...
    entry_points: &[usize],
    bytes: &[u8],
) -> Result<DStatements, DisasmError> {
    let (graph, names) = {
...
fn label_expr(names: &Names, addr: usize) -> Expr {
    names
        .label(addr)
        .map(|name| Expr::name(name))
        .unwrap_or(Expr::num(addr as i32))
}
...
fn lift_instr(
    pos: usize,
    graph: &InstructionGraph<i32, u8>,
    instr: &Instr<i32, u8>,
    names: &Names,
) -> Instr<Expr, IndirectionMode> {
...
fn build_instruction_graph(
    entry_points: &[usize],
    bytes: &[u8],
) -> Result<InstructionGraph<i32, u8>, DisasmError> {
...
impl DStatements {
    ...
    pub fn push_instr_cmt(
        ...
        self.statements
            .push(DStatement::instr(pos, instr).plus_comment(comment));
    }
...
impl DStatement {
    pub fn new(pos: Option<usize>, statement: Statement, comment: Option<String>) -> DStatement {
        DStatement {
            pos,
            statement,
            comment,
    ...
    pub fn bytes(pos: usize, bytes: &[u8]) -> DStatement {
        ...
        DStatement::new(
            Some(pos),
            Statement::Directive(Directive::Byte(Span::default(), vec)),
            if includes_ascii { Some(text) } else { None },
        )
    }
    ...
    pub fn cnop(add: usize, multiple: usize) -> DStatement {
        DStatement::new(
            None,
            Statement::Directive(Directive::Cnop(
                Span::default(),
                Expr::num(add as i32),
                Expr::num(multiple as i32),
            )),
            None,
        )
    }
    ...
    pub fn alias(name: &str, value: Expr) -> DStatement {
        DStatement::new(
            None,
            Statement::Alias(Span::default(), name.to_owned(), value),
            None,
        )
    }
